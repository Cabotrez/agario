...
function GameServer() {
    ...
    this.config = {
        ...
        serverStatsUpdate: 90, // Update interval of server stats in seconds
        ...
        serverRestart: 90, // Toggle whether you want your server to auto restart in minutes. (0 to disable)
        ...
        /** ANTI-BOT **/
        serverIpLimit: 100, // Controls the maximum number of connections from the same IP (0 for no limit)
        ...
        /** BORDER **/
        borderWidth: 24142.135623730952, // Map border size (Vanilla value: 14142)
        borderHeight: 24142.135623730952, // Map border size (Vanilla value: 14142)

        /** FOOD **/
        foodMinSize: 10, // Minimum food size (vanilla 10)
        foodMaxSize: 20, // Maximum food size (vanilla 20)
        foodMinAmount: 2500, // Minimum food cells on the map
        foodMaxAmount: 3500, // Maximum food cells on the map
        foodSpawnAmount: 60, // The number of food to spawn per interval
        ...
        playerMaxNickLength: 45, // Maximum nick length
...
GameServer.prototype.onClientSocketOpen = function (ws, req) {
    ...
    ws.isConnected = true;
    ws.remoteAddress = req.headers['x-forwarded-for'].split(/\s*,\s*/)[0];
...
GameServer.prototype.loadFiles = function () {
    // Load config
    var fs = require("fs");
    var fileNameConfig = this.srcFiles + '/gameserver.mk';
...
GameServer.prototype.pingServerTracker = function () {
    ...
    var data = 'current_players=' + totalPlayers +
        ...
        '&uptime=' + process.uptime() + // Number of seconds server has been runningd
