(function() {
 ...
 (function() {
	...
	const textColoration = (ctx, str, x, y, maxWidth, operation) => {
		let begin;
		if (!str.startsWith("Ǉ")) {
			let i = str.indexOf("Ǉ");
			if (i === -1) i = str.length;
		...
		let sections = str.split("Ǉ").map(s => {
			let color = colors[parseInt(s.charAt(0), 16)];
			return {
				color: color,
				str: color ? s.slice(1) : (s ? "Ǉ" + s : s)
			};
		});
		if (begin) {
			sections.unshift({
		...
		if (align == "right" || align == "end")
			...
			offset = 1/2;
		if (offset) {
			const str = sections.map(s => s.str).join("");
			x -= offset * ctx.measureText(str).width;
		}
		ctx.textAlign = "left";
		for (const s of sections) {
			ctx[operation + "Style"] = s.color;
			ctx[operation + "Text"](s.str, x, y);
			x += ctx.measureText(s.str).width;
		}
		ctx.textAlign = align;
	};
	proto.fillTextFormatted = function(str, x, y, maxWidth) {
		textColoration(this, str, x, y, maxWidth, "fill");
	};
	proto.strokeTextFormatted = function(str, x, y, maxWidth) {
		textColoration(this, str, x, y, maxWidth, "stroke");
	};
	proto.measureTextFormatted = function(str) {
		return this.measureText(str.replace(/Ǉ[\da-f]/gi, ""));
	};
})();
...
//
// var color = 1;
// var x = canvas.width / 2, y = 0;
// ctx.textAlign = "right";
// ctx.fillTextFormatted(`Ǉ1hi, Ǉ2hello, Ǉ3world!`, x, y += 40);
// ctx.textAlign = "center";
// ctx.fillTextFormatted(`Ǉ4hi, Ǉ5hello, 6world!`, x, y += 40);
// ctx.textAlign = "left";
// ctx.fillTextFormatted(`Ǉ7hi, Ǉ8hello, Ǉ9world!`, x, y += 40);
//
// ctx.textAlign = "end";
// ctx.strokeTextFormatted(`Ǉahi, Ǉbhello, Ǉcworld!`, x, y += 40);
// ctx.textAlign = "center";
// ctx.strokeTextFormatted(`Ǉdhi, Ǉehello, Ǉfworld!`, x, y += 40);
// ctx.textAlign = "start";
// ctx.strokeTextFormatted(`hi, Ǉhello, Ǉworld!`, x, y += 40);
// ctx.fillTextFormatted(`hi, hello, world!`, x, y += 40);
    ...
    var wsUrl = null,
        SKIN_URL = "https://speckled-scene.glitch.me/",
    ...
    function wsMessage(data) {
        ...
        switch (packetId) {
            ...
            case 0x63: // chat message
                ...
                var server = !!(flags & 0x100),
                    owner = !!(flags & 0x80),
                    admin = !!(flags & 0x40),
                    mod = !!(flags & 0x20);

                if (server && name !== "SERVER") name = "[SERVER] " + name;
                if (owner) name = "Ǉ5[OWNER] " + name;
                if (admin) name = "Ǉ4[ADMIN] " + name;
                if (mod) name = "Ǉ2[MOD] " + name;
    ...
    var eatSound = new Sound("https://bobsters-agario.000webhostapp.com/eat.mp3", 0.5, 10);
    var pelletSound = new Sound("https://bobsters-agario.000webhostapp.com/pellet.mp3", 0.5, 10);

    request("https://speckled-scene.glitch.me/skinList.txt", function(data) {
    ...
    function buildGallery() {
        ...
        for (var i = 0; i < sortedKeys.length; i++) {
            ...
            c += '<img class="circular" src="https://speckled-scene.glitch.me/' + name + '.png">';
    ...
    function drawChat() {
        ...
        for (var i = 0; i < len; i++) {
            ...
            for (var j = 0; j < complexes.length; j++) {
                ...
                complexes[j].width = ctx.measureTextFormatted(complexes[j].text).width;
        ...
        for (var i = 0; i < len; i++) {
            ...
            for (var j = 0; j < complexes.length; j++) {
                ...
                ctx.fillTextFormatted(complexes[j].text, width, 20 * (1 + i));
    ...
    function drawLeaderboard() {
        ...
        ctx.fillText("Maddest People", 100 - ctx.measureText("Maddest People").width / 2, 40);
        ...
        if (leaderboard.type === "pie") {
            ...
            for (var i = 0; i < len; i++) {
                ...
                ctx.fillTextFormatted(text, start, 70 + 24 * i);
            }
        }
    }
    function drawGrid() {
    ...
    Cell.prototype = {
        ...
        drawText: function(ctx) {
            ...
            if (this.name && settings.showNames) {
              ...
              // ctx.strokeText(ctx.unfomattedText(this.name), this.x, this.y);
             // ctx.fillTextFomatted(this.name, this.x, this.y);
    ...
    function drawTextOnto(canvas, ctx, text, size) {
        ...
        canvas.width = ctx.measureTextFormatted(text).width + 2 * ctx.lineWidth;
       ...
       ctx.strokeStyle = "black";
        ctx.fillTextFormatted(text, 0, 0);
    }
